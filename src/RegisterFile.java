...
public class RegisterFile {
    ...
    public String dump() {
        String res = "";
        res = (String.format("%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%3s%3s%3s%3s", "A", "B", "C", "D", "E", "F", "H", "L", "AF", "BC", "DE", "HL", "PC", "SP", "Z", "N", "H", "C"));
        res += "\n";
        res += (String.format("%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%3s%3s%3s%3s", Integer.toString(A.read(), 16), Integer.toString(B.read(), 16), Integer.toString(C.read(), 16), Integer.toString(D.read(), 16), Integer.toString(E.read(), 16), Integer.toString(F.read(), 16), Integer.toString(H.read(), 16), Integer.toString(L.read(), 16), Integer.toString(AF.read(), 16), Integer.toString(BC.read(), 16), Integer.toString(DE.read(), 16), Integer.toString(HL.read(), 16), Integer.toString(PC.read(), 16), Integer.toString(SP.read(), 16), getFlag(ZFLAG_BIT), getFlag(NFLAG_BIT), getFlag(HFLAG_BIT), getFlag(CFLAG_BIT)));
        return res;
    }
}
